-- 力量动作名统一(第二批):剩余 4 个英文动作名 → 中文规范名
-- 前端配套:src/fit/exerciseLib.ts 的 EXERCISE_ALIASES / EXERCISE_EN(两端映射集合必须一致,增改需同步)
-- 幂等:重复执行无副作用(二次运行时已无英文名可匹配)
-- 前置:20260815120000_normalize_exercise_names.sql 已执行(本批只处理它未覆盖的 4 个名字:
--   chest press/hack squat/rear delt/seated calf,均来自用户「最近常做」快捷方式)
...
insert into alias_map (en, zh, part) values
  ('hack squat', '哈克深蹲', 'legs'),
  ('seated calf', '提踵', 'legs'),
  ('rear delt', '反向飞鸟', 'shoulders'),
  ('chest press', '器械推胸', 'chest');
...
--    仅当原部位为 full_body 时改预设部位,用户显式设置过的保留(如 Rear Delt 的 back)
