...
describe("buildExportPayload", () => {
  it("组装完整导出结构(任务/训练/偏好/体重历史/动作字典/用户信息)", () => {
    const payload = buildExportPayload(
      {
        ...
        user_exercises: [{ name: "卧推", body_part: "chest", bodyweight_default: false, default_reps: 10 }],
      },
      user,
  ...
  it("空数据与无偏好时输出空数组与 null", () => {
    const payload = buildExportPayload(
      { tasks: [], workouts: [], preferences: null, body_weight_history: [], user_exercises: [] },
      user,
      "2026-08-08T00:00:00.000Z",
    );
    ...
    expect(payload.user_exercises).toEqual([]);
  });
});
